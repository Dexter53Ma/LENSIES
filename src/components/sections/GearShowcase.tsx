...
export default function GearShowcase({ data }: GearShowcaseProps) {
  return (
    <section className="relative flex w-full justify-center bg-background px-24 py-80 text-foreground md:px-90 md:py-120">
      <div className="flex w-full max-w-[120rem] flex-col items-center gap-48 md:gap-64">
        ...
        <div className="grid w-full grid-cols-2 gap-16 sm:gap-24 md:grid-cols-3 md:gap-32">
          {data.items.map((item, i) => (
            <Reveal key={item.name} variant="up" delay={(i + 1) as 1 | 2 | 3 | 4 | 5}>
              <div className="flex flex-col gap-12 md:gap-16">
                <div className="img-zoom relative aspect-square w-full overflow-hidden rounded-[1.2rem] bg-cream">
                  <img
                    src={item.image}
                    alt={item.name}
                    width={800}
